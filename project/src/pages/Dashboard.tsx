...
import { getSalesTaskById } from '../lib/leads'; // We will create this function
import TaskDashboardWidget from '../components/TaskDashboardWidget';
import TaskDetailModal from '../components/TaskDetailModal'; // We will create this component
...
function Dashboard() {
  ...
  const [allTeamMembers, setAllTeamMembers] = useState<UserProfile[]>([]); // To pass members to the modal
 ...
 const toggleWidget = (widgetName) => {
  ...
  const loadAllDashboardData = async () => {
    if (!user) return; // Exit if there's no user object yet.
...
useEffect(() => {
  loadAllDashboardData();
   ...
   // This new useEffect will handle opening the task modal from a URL
 useEffect(() => {
 ...
 }, [searchParams, user, allTeamMembers]); // Add allTeamMembers to the dependency array
...
<div className="premium-card p-8 animate-fade-in">
 ...
 <div className="flex justify-end">
   <div className="group relative">
     ...
     <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-gray-800 rounded-md shadow-lg p-2 z-20 hidden group-hover:block border dark:border-gray-700">
  ...
  {/* Column 3: Enhanced Activity Feed */}
