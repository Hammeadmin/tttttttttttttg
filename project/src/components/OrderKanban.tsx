...
import { createQuoteFromLead, acceptQuoteAndCreateOrder } from '../lib/quotes';
...
function OrderKanban() {
 ...
 const loadData = async () => {
  ...
  const handleCreateQuote = async (lead: LeadWithRelations) => {
    if (!lead || !lead.customer_id) {
        showError('Fel', 'Förfrågan måste vara kopplad till en kund för att skapa en offert.');
        return;
    }
    
    try {
        setLoading(true);
        const result = await createQuoteFromLead(lead, DEMO_ORG_ID, lead.assigned_to_user_id);

        if (result.error) {
            throw result.error;
        }

        // Also update the lead's status to 'qualified'
        await updateLead(lead.id, { status: 'qualified' });

        success('Framgång!', 'Offert har skapats och ligger nu som utkast.');
 ...
 const handleDrop = async (e: React.DragEvent, targetStatus: string, targetType: 'order' | 'quote' | 'lead') => {
    ...
    if (acceptError) {
        showError('Fel', `Kunde inte acceptera offerten: ${acceptError.message}`);
    } else {
        success('Framgång!', 'Offerten har accepterats och en order har skapats.');
...
<div className="grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-7 gap-4">
                    ...
                    <>
                      ...
                      {columnLeads.map((lead) => (
                        ...
                        >
                          ...
                          <button
                              onClick={(e) => { // <-- ADD THIS BUTTON
                                e.stopPropagation();
                                handleCreateQuote(lead);
                                console.log('Create Quote for lead:', lead.id);
                              }}
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-lg max-w-2xl w-full p-6">
            <h3 className="text-lg font-semibold">Redigera Förfrågan: {selectedLead.title}</h3>
            {/* Lead Edit Form would go here */}
            <button onClick={() => setShowLeadEditModal(false)} className="mt-4 px-4 py-2 bg-gray-200 rounded-md">Stäng</button>
        </div>
    </div>
)}
...
{showQuoteEditModal && selectedQuote && (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div className="bg-white rounded-lg max-w-2xl w-full p-6">
            <h3 className="text-lg font-semibold">Redigera Offert: {selectedQuote.title}</h3>
            {/* Quote Edit Form would go here */}
            <button onClick={() => setShowQuoteEditModal(false)} className="mt-4 px-4 py-2 bg-gray-200 rounded-md">Stäng</button>
        </div>
    </div>
)}
