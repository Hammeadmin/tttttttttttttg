    ...
    <div className="bg-white rounded-lg shadow-sm border mt-6">
        <div className="px-6 py-4 border-b border-gray-200">
            <h3 className="text-lg font-semibold text-gray-900 flex items-center">
                ...
                Obetalda Provisioner
            </h3>
            <p className="text-sm text-gray-500 mt-1">Lista över slutförda ordrar med provision som väntar på utbetalning.</p>
        </div>
        {commissionOrders.length === 0 ? (
            <EmptyState
                type="general"
                title="Inga provisioner att betala"
                description="Alla provisioner för slutförda ordrar har betalats ut eller så finns inga att visa."
            />
        ) : (
            <div className="divide-y divide-gray-200">
                {commissionOrders.map(order => {
                    const salesperson = teamMembers.find(tm => tm.id === order.primary_salesperson_id);
                    return (
                        <div key={order.id} className="p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between hover:bg-gray-50">
                            <div className="mb-4 sm:mb-0">
                                <p className="font-semibold text-gray-900">{order.title}</p>
                                <p className="text-sm text-gray-600">Säljare: {salesperson?.full_name || 'Okänd'}</p>
                                <p className="text-xs text-gray-500">Order-ID: {order.id}</p>
                            </div>
                            <div className="flex items-center space-x-4">
                                <div className="text-right">
                                    <p className="text-lg font-bold text-green-600">{formatCurrency(order.commission_amount || 0)}</p>
                                    <p className="text-xs text-gray-500">Provision</p>
                                </div>
                                <button
                                    onClick={() => handleMarkCommissionPaid(order.id)}
                                    className="inline-flex items-center px-3 py-1.5 bg-green-100 text-green-700 text-xs font-semibold rounded-md hover:bg-green-200"
                                >
